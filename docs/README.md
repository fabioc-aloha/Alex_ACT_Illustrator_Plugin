...
- **[Flint language reference](../.github/skills/flint-chart/references/flint-language-reference.md)** — shipped Flint 0.5.1 language model, grammar boundaries, compiler pipeline, backend and MCP contract, rendered-demo evidence, and links to the tagged Microsoft sources.
- **[Flint MCP 0.5.1 capability matrix](flint-mcp-0.5.1-capability-matrix.md)** — runtime-captured backend catalog, output, ThemeSpec, MCP App, and Calendar Heatmap boundaries.
