...
> **Current release: v2.5.2.** This PATCH release corrects the Alex Mall update
> contract; the prior MINOR release pins Flint MCP `0.5.1` and adds tested
> Calendar Heatmap authoring for Vega-Lite and ECharts.
...
**Current compatibility:** Core `v4.1.0` provides the baseline runtime; native
...
Use the Mall's dry-run-first `npm run vendor` command with `--replace`, then apply only after Fabio reviews the plan. Run curated maintenance and the full Mall check before committing. The current commands and ownership boundaries live in **[`docs/publishing-to-mall.md`](docs/publishing-to-mall.md)**; do not hand-copy payload files.
