...
- `ascii-chart` skill: renders charts and compact dashboards as pure ASCII on a
  monospace grid. Bar, dot, sparkline, histogram, box plot, scatter, bubble,
  heatmap, funnel, waterfall, treemap, gauge, and KPI forms. It is the only
  delivery path in this plugin with zero runtime dependency: no MCP server, no
  browser, no token. Adopted from the retired `Alex_ACT_Visual_Storytelling`
  plugin under Alex ACT Steward ADR-039.
- Bundled `ascii-chart/references/ascii-gallery.md`: 32 worked forms organized by
  the same seven communication goals as `chart-vocabulary`, a coverage matrix
  mapping 33 Flint chart types to their ASCII counterpart, and an explicit list
  of the forms ASCII cannot carry with what to use instead. Generated by
...
- `chart-vocabulary` routes to `ascii-chart` from the delivery table and the
  selection tree, and its attribution now records that the upstream plugin is
  retired and archived.
- `render-verify` gained an explicit ASCII branch. Steps 1 through 3 assume a
  rendered artifact a browser can open, which ASCII has no equivalent of, so
  `ascii-chart` Module 5 alignment QA substitutes for them. Steps 4 and 5 apply
  unchanged, because checking a claim does not depend on a renderer.
- `flint-chart` section 0.4 now distinguishes its catalog boundary from the
  gallery's medium boundary. A chart absent from Flint may render fine in ASCII,
  and a chart Flint renders well may be impossible in ASCII.

### Removed

- `storytelling-requirements` dropped from `install-visual-companions`, which
